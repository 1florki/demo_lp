...
export class LPGeometry {
  ...
  setVertexColorFrom(heightfield, gradient) {
    let colorFunction = (x, y, z) => {
      return gradient.get(heightfield.getNormalized(x, y, z))
    };
  ...
  createGeometry() {
    ...
    geo.setAttribute('color', new BufferAttribute(bufferedColor, 3));
...
export class LPIcosahedron extends LPGeometry {
  constructor(opts) {
    ...
    if (opts.noise != undefined) {
      this.applyNoiseGradient(opts.noise, opts.gradient);
    }
  }
  createVertices() {
